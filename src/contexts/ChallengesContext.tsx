...
export function ChallengesProvider({
  ...
  function sendLevelBadge() {
    ...
    const badge = badges.filter(
      badge => badge.id === `badgelevel${level + 1}`
    )[0];
  ...
  function sendChallengeBadge() {
    ...
    setIsBadgeModalOpen(true);
    const badge = badges.filter(
      badge => badge.id === `badgechallenges${challengesCompleted + 1}`
    )[0];
    setCurrentBadge(badge);
  ...
  function sendExperienceBadge(xp: number) {
    ...
    setIsBadgeModalOpen(true);
    const badge = badges.filter(
      badge => badge.id === `badgeexperience${xp}`
    )[0];
    setCurrentBadge(badge);
  ...
  function sendCicleBadge() {
    ...
    setIsBadgeModalOpen(true);
    const badge = badges.filter(
      badge => badge.id === `badgecicle${acumullatedChallenges + 1}`
    )[0];
    setCurrentBadge(badge);
  ...
  function levelUp() {
    ...
    axios
      .post('/api/challenges', { id, level: level + 1 })
      .then(response => response.data)
      .catch(err => {
        console.log(err);
      });
  ...
  function completeChallenge() {
    ...
    axios
      .post('/api/challenges', { id, currentExperience: finalExperience })
      .then(response => response.data)
      .catch(err => {
        console.log(err);
      });
